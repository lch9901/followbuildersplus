...
`followbuildersplus` 是一个面向中文用户工作流重新打磨过的 AI builders digest skill。

它的核心目标不是只把链接整理好发给你，而是尽量把“值得看”的信息先整理成可阅读的摘要，再让你通过编号继续追问，直接在聊天里拿到详情甚至原文，而不是被迫跳转到外部链接里自己翻。

对于很多中文用户来说，单纯“给一堆链接”并不是一个友好的阅读体验：

- 外链平台分散，阅读链路长
- 很多内容并不适合频繁跳转查看
- 真正想看的往往只是其中一两条的细节

所以这个版本的设计重点不是“把更多链接发出来”，而是“先帮你筛选，再让你按需展开”。

## 这个版本解决了什么问题

`followbuildersplus` 更适合拿来做“信息收件箱”：

- 先看一份带编号的摘要
- 再挑值得展开的条目继续追问
- 需要时回看当天模型处理前后的本地归档

我们做这个版本，主要就是为了解决下面这些问题：

- 每天固定时间收 AI builders 的信号
- 用统一编号追踪某一条内容
- 需要回看历史条目
- 想知道模型总结之前到底拿到了什么原始数据
- 希望在飞书等聊天工具里直接看详情，而不是频繁点外链

## 核心能力

除了保留原有的 feed 获取、内容重组和消息投递能力，这个版本新增了几层更适合长期使用的能力：
...
很多 AI 信息产品解决的是“发现内容”的问题，但没有真正解决“阅读和追问”的问题。

而在实际使用里，更高频的需求通常是：

- 先快速浏览一遍今天有哪些值得看
- 看到感兴趣的那一条，再进一步展开
- 展开时希望直接在当前聊天里继续读，而不是跳到 X、YouTube 或别的平台

所以 `followbuildersplus` 不是把 digest 当成终点，而是把 digest 当成一个入口。
...
这一步很重要，因为它让“摘要”和“全文/细节”之间的切换发生在同一个聊天窗口里，而不是让用户自己再跳出去找原文。
...
这些能力会逐步增加，但当前版本最重要的目标仍然是：

让你在聊天窗口里完成“浏览摘要 -> 追问详情 -> 查看全文”的闭环。
