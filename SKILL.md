---
name: followbuildersplus
description: AI builders digest with stable item IDs, daily local archives, and detail lookup by full ID like FB20260322001. Use when the user wants numbered builder digests, archived raw materials, or to look up detail for a full followbuildersplus ID.
metadata:
  openclaw:
    requires:
      bins:
...
Check if `~/.followbuildersplus/config.json` exists and has `onboardingComplete: true`.
...
```bash
mkdir -p ~/.followbuildersplus
cat > ~/.followbuildersplus/.env << 'ENVEOF'
...
```bash
cat > ~/.followbuildersplus/config.json << 'CFGEOF'
{
...
openclaw cron add \
  ...
  --session isolated \
  --message "Run the followbuildersplus skill: execute prepare-digest.js, remix the content into a digest following the prompts, keep the provided FB IDs, and return the final digest text" \
  --announce \
...
Read `~/.followbuildersplus/config.json` for user preferences.
...
- `archive` — local archive metadata for the current day
- `rawArchive` — full raw-feed archive metadata for the current day
...
`rawArchive.path` points to the full unfiltered feed snapshot saved immediately
after feed fetch. `archive.path` points to the digest working archive used for
ID-based lookup.

Every candidate item in `x` and `podcasts` already has a stable ID in the
format `FBYYYYMMDDXXX`, such as `FB20260322001`. Preserve that exact ID in the
final digest so the user can look up details later.
The stable ID is in the JSON field named `id`. You must use that field exactly.
Never replace it with tweet `id`, podcast `videoId`, URL fragments, or any other identifier.
...
3. Keep the provided `id`
4. Every tweet MUST include its `url` from the JSON
5. NEVER use the tweet object's own `id` as the bracketed digest ID
...
- The bracketed ID for every selected item must come from the top-level item `id` field only

### Step 4B: Detail Lookup by Full ID

If the user sends a full ID like `FB20260322001`, case-insensitive:

```bash
cd ${CLAUDE_SKILL_DIR}/scripts && node lookup-detail.js FB20260322001
```

Rules:
- Only trigger lookup for a full ID, not for plain numbers like `1`
- `FB`, `fb`, `Fb`, and `fB` should be treated the same
- If lookup returns `not_found`, tell the user the ID is not in the archive
- If found, answer from the archived JSON only and include the original source URLs
- This lookup reads from the digest archive, not the raw archive
...
file to `~/.followbuildersplus/prompts/` and edit it there. This way their
customization persists and won't be overwritten by central updates.

```bash
mkdir -p ~/.followbuildersplus/prompts
cp ${CLAUDE_SKILL_DIR}/prompts/<filename>.md ~/.followbuildersplus/prompts/<filename>.md
```

Then edit `~/.followbuildersplus/prompts/<filename>.md` with the user's requested changes.
...
- "Reset to default" → Delete the file from `~/.followbuildersplus/prompts/`
